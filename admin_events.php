...
if (!empty($_SESSION['admin'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
        ...
        if ($datum && $opis) {
            $events = loadEvents();
            $events[] = ['datum' => htmlspecialchars($datum), 'opis' => htmlspecialchars($opis)];
            saveEvents($events);
            $message = 'Event dodan!';
        }
    }
    ...
    <form method="POST">
      ...
      <div class="add-row">
        <div class="form-group" style="margin:0">
          <label>Datum (npr. JUL 26)</label>
          <input type="text" name="datum" placeholder="AVG 10" maxlength="10" required>
        </div>
        <div class="form-group" style="margin:0">
      ...
      <p class="hint">Format datuma: kratica meseca + dan, npr. "JUN 14", "AVG 02"</p>
    </form>
  </div>
    ...
    <?php else: ?>
      <?php foreach ($events as $i => $ev): ?>
        <div class="event-row">
          ...
          <span class="event-datum"><?= htmlspecialchars($ev['datum']) ?></span>
